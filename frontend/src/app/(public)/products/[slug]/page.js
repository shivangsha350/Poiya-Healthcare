  ...
  if (error || !product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-[#EAF6FB] px-5">
        ...
        <Link href="/products" className="bg-mid text-white px-6 py-2.5 rounded-xl text-sm font-bold shadow hover:bg-[#005f92] transition">
          Back to Catalog
        </Link>
      </div>
    );
  ...
  return (
    <main className="bg-[#EAF6FB] min-h-screen pb-16">
      ...
      <div 
        ...
        style={{ backgroundImage: `url(${DefaultImg.src || DefaultImg})` }}
      >
        <div className="max-w-7xl mx-auto flex flex-col items-start gap-2.5 relative z-10">
          <Link href="/products" className="text-xs text-accent hover:underline font-bold uppercase tracking-wider flex items-center gap-1.5 mb-2">
            <span>← Catalog</span>
          </Link>
            ...
            {related.map((prod) => {
              ...
              return (
                <div key={prod._id} className="border border-slate-200/80 rounded-2xl p-5 bg-white flex flex-col justify-between hover:shadow-lg transition">
                  ...
                  <Link href={`/products/${prod.slug}`} className="text-xs text-mid font-bold mt-4 inline-block hover:underline">
                    View Data Sheet →
                  </Link>
                </div>
              );
